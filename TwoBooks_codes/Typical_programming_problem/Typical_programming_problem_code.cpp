...
//***begin和end都指的是索引值***  (//面试题76：数组中第k大的数字  使用了STL的partition())
...
void QuickSort(int data[], int length, int begin, int end)
{
    ...
    //理解：
    //如果大于，则说明左侧部分至少有两个以上的元素，需要继续对其进行排序。
    // 所以我们递归地调用QuickSort()函数，对左侧部分进行排序。
    //如果小于，则说明右侧部分至少有两个以上的元素，需要继续对其进行排序。
    // 所以我们同样递归地调用QuickSort()函数，对右侧部分进行排序。
    
    //第一个if() index值将不断逼近begin值
    //第二个if() index值将不断逼近end值

    //直到begin和end相等，或者begin大于end，整个数组就完成了排序
#endif
    ...
    if (begin < end)
    {
        //index表示划分后左侧部分的结束位置(右侧部分的开始)。
        int index = Partition(data,length,begin,end);
        //递归地调用QuickSort()函数对划分后的左侧部分进行排序
        QuickSort(data,length,begin,index-1);
        //递归地调用QuickSort()函数对划分后的右侧部分进行排序
        QuickSort(data,length,index+1,end);
    }
#endif
}
...
{
    //dummy 节点充当哨兵节点，它的作用是简化链表操作。
    //通过使用哨兵节点，我们可以避免在合并链表时处理头节点的
    //特殊情况，从而简化代码逻辑
    ListNode dummy(0);
    ListNode* tail = &dummy;
...
bool StackWithMin<T>::push(const T& item)
{
    ...
    // 则这样的成员在派生类的方法中是可用的，但在继承层次结构外是不可用的。
    // 如果使用组合将这样的类包含在另一个类中，则后者将不是派生类，
    // 而是位于继承层次结构之外，因此不能访问保护成员。
    // 那就只能通过对象来访问该被包含的类对象对应的类的方法。
...
const int N = 8; // Number of queens and the size of the chessboard (NxN)

// Check if it's safe to place a queen at the given position
bool isSafe(int row, int col, vector<string>& board) 
{
    // Check this row on left side 检查行
    for (int i = 0; i < col; i++)
        if (board[row][i] == 'Q')
            return false;

    // Check upper diagonal on the left side 检查“\”斜对角
    for (int i = row, j = col; i >= 0 && j >= 0; i--, j--)
        if (board[i][j] == 'Q')
            return false;

    // Check lower diagonal on the left side  检查“/”斜对角
...
bool solveNQUtil(int col, vector<string>& board) 
{
...
void GetLeastNumbers(const vector<int>& data, int k)
{
    ...
    //***注***
    //priority_queue容器的元素默认
    // 从左往右是从小到大存放    (less<>)

    //***以下是改为由大到小存放***
    //priority_queue<int, vector<int>, greater<int>> pq;
    //理解：
    //想象一个长方形的priority_queue容器，从左侧推入元素，
    //从右侧弹出元素，用greater<int>实参后，每两个元素之间
    //关系为 >
    //那么每次弹出的都是当前容器状态下最小的元素
...
if(data[i]>=0)
dp[i]=max(data[i],dp[i-1]+data[i]-NegSum)
else
NegSum+=data[i]
dp[i]=dp[i-1]
...
int FindGreatestSum(vector<int>& data)
{
